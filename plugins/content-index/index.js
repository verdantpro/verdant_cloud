...
// VENDOR PATCH: keep the RSS feed to actual notes. the stock feed listed folder
// listings ("notes") and every tag page ("cloud", "Tag Index", …) as items, plus
// the homepage — each dated `new Date()` since generated/undated pages carry no
// frontmatter date, so they sorted to the TOP, ahead of real notes, and re-dated
// to "today" on every build (noise for subscribers). this drops tag pages, any
// folder-index listing (slug ending "/index"), and the bare homepage ("index").
// only RSS is affected — the sitemap and contentIndex.json still index
// everything from the shared loop below.
function isNonNoteSlug(slug2) {
  return (
    slug2 === "index" ||
    slug2 === "tags" ||
    slug2.startsWith("tags/") ||
    slug2.endsWith("/index")
  );
}
function generateRSSFeed(cfg, idx, options, limit) {
  ...
  const items = Array.from(idx).filter(([slug2]) => !isNonNoteSlug(slug2)).sort(([_2, f1], [__, f2]) => {
    if (f1.date && f2.date) {
